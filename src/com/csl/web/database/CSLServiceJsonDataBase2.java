...
import com.xcsl.miniserver.help.JsonCmdHelp;

public class CSLServiceJsonDataBase2 implements ICSLService {

	
	private static   String datafiledir = System.getProperty("user.dir")+FileUtils
			.separatorChar+"datafile";
	...
	public void printMessage(String s) {
		...
		System.out.println("[JSONDATABASE]"+s);
	}
	...
	public IFileStoreService getFileStoreService() {
		...
		return CSLContext.instance.getIDSMainProcessor().getFileStoreServices();
...
//				);
	...
	private  Json writeDataFile(String path, String content) {

		path=datafiledir+File.separator+path;
		
		return getFileStoreService().writeFileFromText(path, content);
...
//	}
	...
	private  Json readDataFile(String path) 
	{
		...
		path=datafiledir+FileUtils.separatorChar+path;
		System.out.println("READING_FILE:"+datafiledir);

		Json z= getFileStoreService().readFileInAJsonText(path);
		...
		String s="";
		if (z.has("text")) {
			s=z.get("text").asString();
			result.set("contents", Json.read(s));
		}
		
		if (z.has("error"))  result.set("error", z.get("error"));
	...
	public Json saveJsonAsDataFile(String fileName,Json data, boolean pretty) {
...

		{
			...
			if ((fileName!=null)&&(contents!=null)) {
				Json jresult=writeDataFile(fileName+".json", s);
				//sendEventFileModified(fileName, uuid);
				j.set("result",jresult);
