...
public class CSLDemo02JSONServer {
	...
	static public Json readObjectFromDatabase(String name) {
		HttpGet get = new HttpGet(getServerURL()+"load_jsonfile/"+name);
	...
	static public Json writeObjectToDatabase(String filename, Json contents) {
		...
		HttpPost post = new HttpPost(getServerURL()+"save_jsonfile/"+filename);
		HttpClient  client    = HttpClientBuilder.create().build();
		...
		try {
			...
			String  result = sb.toString();
			System.out.println("tesul="+result);
	...
	static public void testLoadJsonFile() {
