...
function realTimeLineChart() {
  ...
  function chart(selection) {
    ...
    selection.each(function(data) {
      data = ["input", "output", "setpoint"].map(function(c) {
        return {
          label: c,
    ...
    /*		  [
        ...
        d3.max(data, function(c) { return d3.max(c.values, function(d) { return d.value; })})+1
      ]);*/
     ...
     g.selectAll("g path.data")
      ...
      function tick() {
        ...
        var xMinLess = -1 //new Date(new Date(xMin).getTime() - duration);
        d3.active(this)
