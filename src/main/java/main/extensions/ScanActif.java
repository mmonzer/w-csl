...
public class ScanActif {
	 ...
	 */
	public ScanActif(boolean debug, boolean verbose, String tap, boolean debugMode, boolean logMode, String debugPath, String logPath) {
        ...
        print("Débug (local) : "+debug,2);
        print("Debug (config) : "+debugMode,2);
        print("Debug repo : "+debugPath,2);
        print("Log : "+logMode,2);
        print("Log path: "+logPath,2);
		...
		try {
			...
			print("Scan de "+currentNetwork,2);
				...
				for(Json j : configuredTaps) {
					if(j.at("idname").asString().contentEquals(tap)) {
						String ipadd = j.at("ip").asString();
				...
				output = readFile(debugPath+"scan_net_"+ip+".xml");
			}
			if(logMode) {
				writeFile(logPath+"scan_net_"+ip+".xml", output);
			}
		} catch (Exception e) {
			e.printStackTrace();
		}
	 ...
	 */
	private Json scanMachine(String ip) throws JSchException, IOException {
		Json result = Json.object();
		String output;
		if(!debugMode) {
			String username = "", password ="";
			...
			output = readFile(debugPath+"scan_dev_"+ip+".xml");
		}
		if(logMode) {

			writeFile(logPath+"scan_dev_"+ip+".xml", output);
		}
