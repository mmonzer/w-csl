...
public abstract class CouchDbClientBase {
	...
	public Json findAsJson( String id) {
	...
	public Response save(Json object) {
		return put(getDBUri(), object, true);
	}
	...
	public Response update(Json object) {
		return put(getDBUri(), object, false);
	}
	...
	private String b2s(InputStream stream) throws IOException {
	...
	Response put(URI uri,Json json, boolean newEntity) {
		//assertNotEmpty(object, "object");
		HttpResponse response = null;
		try {  
			//final JsonObject json = getGson().toJsonTree(object).getAsJsonObject();
			String id = JsonUtil.getAsString(json,"_id"); // getAsString(json, "_id");
			String rev = JsonUtil.getAsString(json,"_rev"); // getAsString(json, "_rev");
			if(newEntity) { // save
				assertNull(rev, "rev");
				id = (id == null) ? generateUUID() : id;
			} else { // update
